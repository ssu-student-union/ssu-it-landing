...
| 학과(전공) | rich_text | `stepOne.major` |
| 학년/학기 | select | `stepOne.grade` |
| 개인정보 동의 | checkbox | `stepOne.agree` |
| 면접 가능 날짜 | multi_select | `stepTwo.interviewAvailability`의 key (필터용) |
| 면접 가능 일정 | rich_text | 표준 슬롯 상세 또는 `[대체 일정]` + `stepTwo.otherTime` (통합) |
| 대체 일정 필요 | checkbox | `stepTwo.noAvailableTime` (필터용) |
| 지원 부서 | select | `stepOne.department` |
