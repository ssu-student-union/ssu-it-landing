...
export const ProjectTeam = ({ team }: ProjectTeamProps) => {
  return (
    <div className="flex h-full flex-col gap-4 rounded-[1.25rem] border border-[#cdc4c4] p-6 sm:gap-5 sm:p-8 lg:p-16">
      <div className="flex items-center gap-2 sm:gap-3 lg:gap-0.5">
        <Image
          src={teamIcon}
          alt=""
          width={70}
        ...
        {team.map((member) => {
          ...
          return (
            <div key={`${member.role}-${member.name}`}>
              {hasMultipleNames ? (
                <>
                  <p className="font-bold text-base text-white sm:text-lg lg:text-[1.875rem]">
                    {member.role}
                  </p>
                  <p className="font-bold text-sm text-white sm:text-base lg:text-2xl">
                    {member.name}
                  </p>
                </>
              ) : (
                <p className="font-bold text-base text-white sm:text-lg lg:text-[1.75rem]">
